...
Under a precompactness assumption, we show these maximal Chow-destabilizers
indeed converge to $\Theta_{P}$ as $k\rightarrow\infty$. It may
provide an algorithm to detect K-unstability of toric varieties. 
\end{abstract}
...
(2) We show that the expected convergence holds under a pre-compactness
assumption, see Theorem \ref{thm:main converge} below. 
...
\label{thm:main converge}If the minimizer sequence $\psi_{k}$ (\ref{eq: quantum mini})
is $C^{0}$-precompact, then $\psi_{k}$ converges uniformly to $-\Theta_{P}$,
hence 
\begin{equation}
...
The proof uses a Gamma convergence argument, building on the continuity
of $\Theta_{P}$ and (\ref{eq:EM enhanced}). By Arzelà--Ascoli theorem,
$C^{0}$-precompactness is equivalent to $\{\psi_{k}\}$ being uniformly
bounded and equicontinuous. It needs more delicate analysis on this
sequence, we leave it for the future. 
...
\begin{figure}[h]
\centering
\includegraphics[scale=0.3]{de-pentagon}

\caption{$\Sigma(A)$ and the corresponding subdivisions. }
...
\begin{figure}[h]
\centering
\includegraphics[viewport=0bp 0bp 1150bp 670bp,clip,scale=0.3]{\string"position of Secondary Polytope\string".pdf}\caption{\label{fig: Location of second poly}We only shows the positional
relations. }
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[viewport=150bp 200bp 1140bp 650bp,clip,scale=0.3]{\string"chow stable\string".pdf}\caption{\label{fig: stable}Left: $(Q,A)$ is Chow-semistable. Right: $(Q,A)$
is Chow-unstable but relatively Chow-semistable. Refer to Figure \ref{fig: Location of second poly}. }
\end{figure}
\end{thm}
...
\begin{figure}[h]
\centering
\includegraphics[viewport=200bp 120bp 800bp 620bp,clip,scale=0.3]{\string"example mGKZ\string".pdf}

\caption{\label{fig: SA}subdivision $S(\sigma_{A})=S_{A}$}
\end{figure}
...
\label{proof of convergence} We use a Gamma-convergence argument.
We choose the function space $\mathcal{C}(P)$ endowed with the $C^{0}$-norm.
Functional $\Psi_{k}$ pointwisely converges to $\Psi$ by (\ref{eq:EM}).
We know that $\Psi_{k}$ admits a unique minimizer $\psi_{k}$ by
Theorem \ref{thm: min Psi_k INTRO}, and $\Psi$ also admits a unique
minimizer $\psi\coloneqq-\Theta_{P}\in\mathcal{C}(P)$ by Proposition
\ref{prop: recharac Theta} and Theorem \ref{thm: theta is continuous-INTRO}. 

First, since $\Psi_{k}(\psi_{k})\leq\Psi_{k}(\psi)$, we have 
\[
\limsup_{k\rightarrow\infty}\Psi_{k}(\psi_{k})\leq\lim_{k}\Psi_{k}(\psi)=\Psi(\psi)=\inf\Psi.
\]
If one can show that each subsequence of $\psi_{k}$ has a subsequence
which converges to $\psi$, then it follows that $\psi_{k}$ converges
to $\psi$ via a contradiction argument. Without loss of generality,
we consider the sequence $\psi_{k}$ itself. 

By the \textit{pre-compactness assumption}, $\psi_{k}$ has a subsequence
$\psi_{k_{j}}$ which uniformly converges to $\psi_{\infty}\in\mathcal{C}(P)$.
It remains to show $\psi_{\infty}=\psi$. By (\ref{eq:EM enhanced}),
we have 
\[
\liminf_{j\rightarrow\infty}\Psi_{k_{j}}(\psi_{k_{j}})=\Psi(\psi_{\infty})\geq\Psi(\psi)=\inf\Psi.
\]
We already show $\limsup_{j}\Psi_{k_{j}}(\psi_{k_{j}})\leq\inf\Psi$.
It forces $\Psi(\psi_{\infty})=\Psi(\psi)$, so $\psi_{\infty}=\psi$
by the uniqueness of minimizer of $\Psi$. 
\end{proof}

\begin{rem}
We have a plan to verify the precompactness of $\psi_{k}=2V_{P}k^{n+1}\sigma^{\vee}_{k}-2k$. 
...
then drop the assumption of Theorem \ref{thm:main converge}. 
