...
We use the following standard compactness form of the convex quadrature
lemma. Notice that no control of the values on faces of codimension
at least two is assumed.

\begin{lem}[convex quadrature compactness]
\label{lem: convex quadrature compactness}Assume that $P$ is Delzant.
There is a constant $C_{P}$ such that
\begin{equation}
B_{k}(f)\geq-C_{P}\left\Vert f\right\Vert _{k,2},\qquad
\left\Vert f\right\Vert _{L^{2}(P)}\leq C_{P}\left\Vert f\right\Vert _{k,2}
\label{eq: convex quadrature coercivity}
\end{equation}
for every sufficiently large $k$ and every $f\in\mathcal{C}(P)$. Since
only the limit $k\rightarrow\infty$ is used below, the finitely many
smaller values of $k$ are irrelevant. Moreover, suppose that
$f_{k}\in\mathcal{C}(P)$ and $\sup_{k}\left\Vert f_{k}\right\Vert _{k,2}<\infty$.
After passing to a subsequence, there is a convex $f\in L^{2}(P)$
such that $f_{k}\rightharpoonup f$ weakly in $L^{2}(P)$ and
$f_{k}\rightarrow f$ locally uniformly on $\mathrm{int}P$. We use
the lower semicontinuous extension of $f$ to $P$ and allow its boundary
integral to be $+\infty$. For this subsequence,
\begin{equation}
\liminf_{k\rightarrow\infty}\left\Vert f_{k}\right\Vert _{k,2}^{2}
\geq\left\Vert f\right\Vert _{L^{2}(P)}^{2},\qquad
\liminf_{k\rightarrow\infty}B_{k}(f_{k})
\geq\int_{\partial P}f\,\mathrm{d}\sigma.\label{eq: convex quadrature liminf}
\end{equation}
If, in addition,
\begin{equation}
\lim_{k\rightarrow\infty}\left\Vert f_{k}\right\Vert _{k,2}^{2}
=\left\Vert f\right\Vert _{L^{2}(P)}^{2},\label{eq: no discrete L2 defect}
\end{equation}
then $f_{k}\rightarrow f$ strongly in $L^{2}(P)$.
\end{lem}

\begin{proof}
We divide the proof into four steps. Write the facets as
...
constants depend only on $P$. The Delzant assumption is used only in
the lattice trapezoid calculation below.

\textbf{Step 1: sampling estimates for convex functions.}
...
We also need to control the negative part. We first spell out the
geometric observation used for this purpose. There are constants
$A$ and $k_{0}$ such that, if $k\geq k_{0}$ and
$\Delta\in\mathcal{T}_{k}$ has vertices $u_{0},\ldots,u_{n}$, one
can find $p=p(\Delta)\in P_{k}$ satisfying
\begin{equation}
2p-u_{i}\in P_{k},\qquad
\operatorname{dist}(p,\Delta)+
\max_{i}\operatorname{dist}(2p-u_{i},\Delta)\leq A/k.
\label{eq: reflected lattice stencil}
\end{equation}
Here is the construction. Let $I(\Delta)$ consist of those $r$ for
which $\min_{\Delta}l_{r}\leq A_{0}/k$. If $I(\Delta)$ is empty,
take $p=u_{0}$. The diameter bound in property (2), with $A_{0}$
sufficiently large, then shows that every reflection $2p-u_{i}$ lies
in $P$. If $I(\Delta)$ is nonempty, then, for all sufficiently large
$k$, the facets $F_{r}$ with $r\in I(\Delta)$ have a common face
$G$: a collection of facets with empty intersection has a fixed
positive lower bound for $\max_{r}l_{r}$ on the compact set $P$.
Choose once and for all an integral vector $w_{G}$ pointing strictly
into every facet containing $G$. Put
$p=u_{0}+a w_{G}/k$, where the fixed integer $a$ is larger than the
maximum, over the finitely many rescaled simplex shapes, of the
oscillation of each defining function $l_{r}$ on that simplex. Then
\[
l_{r}(2p-u_{i})=2l_{r}(p)-l_{r}(u_{i})\geq0
\]
for every facet containing $G$. For $r\notin I(\Delta)$, the negative
change caused by the fixed vector $2a w_{G}/k$ is bounded by $C/k$,
whereas $l_{r}\geq A_{0}/k$ on $\Delta$. Increasing $A_{0}$ once
therefore gives the same inequality for those facets. Since there are
only finitely many faces and
rescaled simplex shapes, uniform $a,A$ and $k_{0}$ may be chosen.
This proves (\ref{eq: reflected lattice stencil}).

Write $x=\sum_{i}\lambda_{i}u_{i}$ and set
$y=2p-x=\sum_{i}\lambda_{i}(2p-u_{i})$. Convexity, first on the
segment $[x,y]$ and then on the reflected simplex, gives
\[
h(p)\leq\frac{h(x)+h(y)}{2},\qquad
h(y)\leq\sum_{i=0}^{n}\lambda_{i}h(2p-u_{i}).
\]
It follows that
\[
h(x)\geq2h(p)-\sum_{i=0}^{n}\lambda_{i}h(2p-u_{i})
\]
and hence
\begin{equation}
h_{-}(x)^{2}\leq C\left(h(p)^{2}
+\sum_{i=0}^{n}h(2p-u_{i})^{2}\right).
\label{eq: negative pointwise sampling}
\end{equation}
The points on the right form a stencil of uniformly bounded size in
the $A/k$-neighborhood of $\Delta$. Each lattice point occurs in only
a uniformly bounded number of stencils, because the mesh has bounded
overlap and all its simplices have diameter $O(k^{-1})$. Integrating
(\ref{eq: negative pointwise sampling}), adding
(\ref{eq: positive sampling estimate}), and summing over the relevant
simplices gives
\begin{equation}
\int_{E}h^{2}\,\mathrm{d}x
\leq\frac{C}{k^{n}}
\sum_{\substack{u\in P_{k}\\ \operatorname{dist}(u,E)\leq C/k}}
h(u)^{2}\label{eq: local sampling estimate}
\end{equation}
for every union $E$ of simplices of $\mathcal{T}_{k}$. Approximating
an arbitrary measurable set from outside by such a union only changes
$C/k$ to $2C/k$. Taking $E=P$ proves
\begin{equation}
...
We shall also use the following interior estimate. If
$K\Subset K'\Subset\mathrm{int}P$ and $h$ is convex, then
\begin{equation}
\sup_{K}|h|+\operatorname{Lip}_{K}(h)
\leq C_{K,K'}\left\Vert h\right\Vert _{L^{2}(K')}.
\label{eq: interior convex estimate}
\end{equation}
For completeness, choose $r>0$ so that every ball of radius $4r$
centered in $K$ is contained in $K'$. Jensen's inequality on
$B(x,r)$ gives
\[
h(x)\leq |B(x,r)|^{-1}\int_{B(x,r)}h
\leq C_{r}\left\Vert h\right\Vert _{L^{2}(K')}.
\]
...
$z$ varies, and therefore $M\leq C_{r}\Vert h\Vert_{L^{2}(K')}$.
If $M\leq2A$, the same conclusion is immediate. Thus $|h|$ is
bounded on the $2r$-neighborhood of $K$. Finally, if
$q\in\partial h(x)$ and $x\in K$, the subgradient inequalities at
$x\pm r e_{i}$ give
\[
h(x)\pm r\langle q,e_{i}\rangle\leq h(x\pm r e_{i}).
\]
The $C^{0}$ bound therefore bounds all components of $q$, and the
subgradient inequality on line segments gives the Lipschitz estimate
in (\ref{eq: interior convex estimate}).

We next prove the first estimate for $B_{k}$. Fix
$p\in\mathrm{int}P$ and choose a supporting affine function
\[
\ell(x)=h(p)+\langle q,x-p\rangle,\qquad q\in\partial h(p),
\]
and set $g=h-\ell\geq0$. Apply
(\ref{eq: interior convex estimate}) to a compact neighborhood of
$p$, and then use (\ref{eq: detailed discrete continuous bound}).
The value $h(p)$ and every component of $q$ are bounded by
$C\Vert h\Vert_{k,2}$. Since $P$ is bounded, this proves
\[
\Vert\ell\Vert_{C^{0}(P)}\leq C\Vert h\Vert_{k,2}.
\]
Moreover, Ehrhart's estimate $k^{-n}|P_{k}|\leq C$ implies
$\Vert\ell\Vert_{k,2}\leq C\Vert\ell\Vert_{C^{0}(P)}$. The discrete
triangle inequality for $g=h-\ell$ now gives
\begin{equation}
\left\Vert\ell\right\Vert _{C^{0}(P)}
+\left\Vert g\right\Vert _{k,2}
\leq C\left\Vert h\right\Vert _{k,2}.
\label{eq: supporting affine control}
\end{equation}

We now give the lattice trapezoid calculation which will be used both
here and in Step 2. If compact polytopes
$K_{F}\Subset\operatorname{relint}F$ are prescribed, then there are
fixed compact convex sets
$Q\Subset Q'\Subset\operatorname{int}P$ such that every
nonnegative convex $g$ satisfies
\begin{equation}
\sum_{u\in P_{k}}g(u)-k^{n}\int_{P}g\,\mathrm{d}x
\geq\frac{1}{2}\sum_{F}
\sum_{u\in K_{F}\cap P_{k}}g(u)
-Ck^{n-2}\left(\sup_{Q}g+\operatorname{Lip}_{Q}(g)\right).
\label{eq: lattice trapezoid estimate}
\end{equation}
When $n=1$, the last term is absent.

Here is the cell bookkeeping behind this estimate. At a codimension
$r$ face of a Delzant polytope, the primitive defining functions of
the $r$ facets through that face extend to integral affine
coordinates. In these coordinates a sufficiently small face
neighborhood is an orthant, and the coordinate change has determinant
$\pm1$. Thus it preserves both the lattice and the volume form.
Subdivide the neighborhood into half-open lattice boxes
of side $1/k$. If $R$ is one such box, with vertices
$v_{\varepsilon}$, $\varepsilon\in\{0,1\}^{n}$, every $x\in R$ is
the convex combination of these vertices with the multilinear
barycentric weights. More explicitly, in the box coordinates put
$t_i=k(x_i-a_i)\in[0,1]$ and
$\lambda_{\varepsilon}(x)=\prod_i t_i^{\varepsilon_i}
(1-t_i)^{1-\varepsilon_i}$. Then
$\int_R\lambda_{\varepsilon}\,\mathrm{d}x=2^{-n}k^{-n}$.
Integrating the convexity inequality gives
\begin{equation}
k^{n}\int_{R}g\,\mathrm{d}x
\leq2^{-n}\sum_{\varepsilon\in\{0,1\}^{n}}
g(v_{\varepsilon}).
\label{eq: box trapezoid}
\end{equation}
After summing (\ref{eq: box trapezoid}), an interior mesh point occurs
in $2^{n}$ boxes and has total weight $1$; a point in the relative
interior of a facet occurs in $2^{n-1}$ boxes and has weight $1/2$;
more generally, a point in the relative interior of a codimension
$r$ face has weight $2^{-r}$. Each of these weights is no larger than
the coefficient $1$ with which that point occurs in
$\sum_{u\in P_{k}}g(u)$.

For clarity, the global cell decomposition is obtained as follows.
First make the box decomposition on every face and extend it
successively in the inward normal directions. At level $k$, choose
each artificial separating interface on the nearest lattice layer
and triangulate the one-layer slab between two different box charts.
Only finitely many rescaled slab shapes occur. Assign one half of the
lattice points on such an interface to each adjacent chart. For a
constant function their two half-weights add exactly to the interior
weight one. For a general $g$, subtract its value at one vertex of
each slab cell. The constant parts still cancel, whereas the remaining
part is bounded by $Ck^{-1}\operatorname{Lip}_{Q}(g)$ per cell. Since
there are $O(k^{n-1})$ cells along all codimension-one interfaces,
their total contribution is at most
$Ck^{n-2}\operatorname{Lip}_{Q}(g)$.

Fill the remaining core by lattice boxes and triangulate the finitely
many cut-cell shapes. A cut cell which is not already included in a
codimension-one slab must meet two separating interfaces, so there
are only $O(k^{n-2})$ such cells. All their vertices lie in one fixed
compact set $Q\Subset\operatorname{int}P$, and their rescaled volumes
and numbers of vertices are uniformly bounded. Their total error is
therefore at most $Ck^{n-2}\sup_{Q}g$. The genuine boundary
coefficients are nonnegative, and on each prescribed $K_{F}$ they
equal $1/2$ for all sufficiently large $k$. Dropping all remaining
nonnegative coefficients proves
(\ref{eq: lattice trapezoid estimate}).

Take all $K_{F}$ empty in
(\ref{eq: lattice trapezoid estimate}), multiply by $2/k^{n-1}$,
and use (\ref{eq: interior convex estimate}) and
(\ref{eq: detailed discrete continuous bound}). This gives
\begin{equation}
B_{k}(g)\geq-C\left\Vert g\right\Vert _{k,2}.
\label{eq: nonnegative quadrature lower bound}
\end{equation}
For an affine function $\ell$, the first two coefficients of the
weighted Ehrhart polynomial give
\[
B_{k}(\ell)=\int_{\partial P}\ell\,\mathrm{d}\sigma
+O(k^{-1})\left\Vert\ell\right\Vert _{C^{0}(P)}.
\]
Since $B_{k}$ is linear, combining this identity with
(\ref{eq: supporting affine control}) and
(\ref{eq: nonnegative quadrature lower bound}) proves
\[
B_{k}(h)\geq-C\left\Vert h\right\Vert _{k,2}.
\]
Together with (\ref{eq: detailed discrete continuous bound}), this
proves (\ref{eq: convex quadrature coercivity}).

\textbf{Step 2: the boundary liminf.}
We first establish the compactness needed in this step. By
(\ref{eq: detailed discrete continuous bound}), the sequence
...
For each $K\Subset K'\Subset\mathrm{int}P$, the right-hand side of
(\ref{eq: interior convex estimate}) is uniformly bounded. Hence
$\{f_{k}\}$ is uniformly bounded and equicontinuous on $K$. Choose
an exhaustion
...
-\frac{C}{k}\left(\sup_{Q}g_{k}
+\operatorname{Lip}_{Q}(g_{k})\right).
...
$2$ in $B_{k}$. The error is $O(k^{-1})$ whenever $g_{k}$ is locally
uniformly bounded on $\mathrm{int}P$, because
(\ref{eq: interior convex estimate}) also gives a uniform Lipschitz
bound on $Q$.

We apply this to the subsequence just constructed. Choose the same
point $p\in\mathrm{int}P$ as above and
...
(\ref{eq: facet lattice lower bound}) and the boundedness of
$\sup_{Q}g_{k}+\operatorname{Lip}_{Q}(g_{k})$,
\[
...
\textbf{Step 3: the quadratic liminf.}
...
\]
Together with (\ref{eq: detailed boundary liminf}), this proves
(\ref{eq: convex quadrature liminf}).

\textbf{Step 4: absence of an $L^{2}$ boundary defect.}
...
\end{align}
Apply the local sampling estimate
(\ref{eq: local sampling estimate}) to
$E=P\setminus P^{2\delta}$. For all sufficiently large $k$, the
$C/k$-neighborhood of $E$ is contained in
$P\setminus P^{\delta}$. Thus
\[
\limsup_{k\rightarrow\infty}
\int_{P\setminus P^{2\delta}}f_{k}^{2}\,\mathrm{d}x
\leq C\int_{P\setminus P^{\delta}}f^{2}\,\mathrm{d}x.
\]
...
&\leq2C\int_{P\setminus P^{\delta}}f^{2}\,\mathrm{d}x
+2\int_{P\setminus P^{2\delta}}f^{2}\,\mathrm{d}x.
\end{align*}
Both terms tend to zero as $\delta\downarrow0$, because
...
\ref{lem: convex quadrature compactness} cannot be omitted for a
general lattice polytope. For example, let
\[
...
in (\ref{eq: convex quadrature liminf}) would read
...
\end{equation}
Lemma \ref{lem: convex quadrature compactness}, applied to
...
Indeed, Lemma \ref{lem: convex quadrature compactness} interprets this
integral in the extended sense. If it were $+\infty$, then
(\ref{eq: choose discrete norm limit}) and
(\ref{eq: convex quadrature liminf}) would imply
\[
...
(\ref{eq: convex quadrature liminf}) give
\begin{align*}
\liminf_{j\rightarrow\infty}\Psi_{k_{j}}(\psi_{k_{j}})
...
which is the first inequality in (\ref{eq: convex quadrature liminf}).
...
Lemma \ref{lem: convex quadrature compactness}, after passing to a
...
liminf in (\ref{eq: convex quadrature liminf}) would contradict
...
=\left\Vert\psi\right\Vert _{L^{2}(P)}^{2}.
\]
The quadratic liminf in (\ref{eq: convex quadrature liminf}) first
shows that
$\left\Vert\psi\right\Vert _{L^{2}(P)}^{2}
...
\]
Therefore the no-defect hypothesis
(\ref{eq: no discrete L2 defect}) in the last assertion of
Lemma \ref{lem: convex quadrature compactness} is satisfied. That
assertion yields
\[
...
\begin{thm}
Let $P\subset\mathbb{R}^{n}$ be a lattice polytope. For any $k\in\mathbb{Z}_{>0}$,
...
each facet $F\subset\partial P$ spanning affine subspace $\mathrm{Aff}(F)$,
$\mathrm{d}\sigma|_{F}$ is the Lebesgue measure normalized by the
lattice $\mathrm{Aff}(F)\cap\mathbb{Z}^{n}$. 
\end{thm}

\begin{rem}
(1) when $f$ is smooth on an open set containing $P$, (\ref{eq:EM})
is just the leading two-terms of the Euler--Maclaurin expansion.
There is vast literature discussing it, see \cite{tate_asymptotic_2011}
and its references. (2) if $f$ is merely continuous, (\ref{eq:EM})
